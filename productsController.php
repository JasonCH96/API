...
switch($_SERVER['REQUEST_METHOD']){
    ...
    case 'POST':
        ...
        if(!isset($_POST->name) || is_null($_POST->name) || empty(trim($_POST->name)) || strlen($_POST->name) > 80){
            $respuesta= ['error','El nombre del producto no debe estar vacío y no debe de tener mas de 80 caracteres'];
        }
        else if(!isset($_POST->description) || is_null($_POST->description) || empty(trim($_POST->description))){
            $respuesta= ['error','La descripcion del producto no debe estar vacía'];
        }
        else if(!isset($_POST->price) || is_null($_POST->price) || empty(trim($_POST->price)) || !is_numeric($_POST->price) || strlen($_POST->price) > 20){
            $respuesta= ['error','El precio del producto no debe estar vacío, debe ser de tipo numerico y no tener mas de 20 caracteres'];
        }
        else{
    ...
    case 'PUT':
        ...
        if(!isset($_PUT->id) || is_null($_PUT->id) || empty(trim($_PUT->id))){
            ...
            $respuesta= ['error','La descripcion del producto no debe estar vacío'];
        }
